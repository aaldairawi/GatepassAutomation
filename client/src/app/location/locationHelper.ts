...
export const locationHelper  = (berth: string, lastSlotPosition: string) : string  => {
    
    let  yardLocationSubstring : string = "";    
    const ENHAM_SUB_LOCATION_LOCATIONS = ["H1", "H2", "I1", "I2", "M1", "M2","N1","N2","O1","O2","J1","J2","K1","K2","EN"];
    const YARD_20_SUB_LOCATION_LOCATIONS = ["B1", "B2", "B3","C1","C2","C3","D1","D2","D3","D4","E1","E2","E3","F1","F2","F3","F4","G1","G2","G3","OO","G4"];


    if(!lastSlotPosition)
    {
        return `N/A/${lastSlotPosition}`;
    } else {
        yardLocationSubstring = lastSlotPosition.substring(0, 2);
    }
    if(berth.length > 0 && berth === "B27")
        return `B27/${lastSlotPosition}`;
    if(berth.length > 0 && berth === "B20")
        return `B20/${lastSlotPosition}`;
    if(lastSlotPosition.length > 0 && lastSlotPosition === "YARD21")
        return `B21/${lastSlotPosition}`;
    if(lastSlotPosition.length > 0 && lastSlotPosition === "ENHAM" || ENHAM_SUB_LOCATION_LOCATIONS.some((element) => element === yardLocationSubstring))
        return `ENHAM/${lastSlotPosition}`;
    if(lastSlotPosition.length > 0 && lastSlotPosition === "SHEBEL" || yardLocationSubstring === "SH")
        return `SHEBEL/${lastSlotPosition}`;
    if(lastSlotPosition.length > 0 && YARD_20_SUB_LOCATION_LOCATIONS.some((element) => element === yardLocationSubstring))
        return `B20/${lastSlotPosition}`;
    return "N/A";
};
